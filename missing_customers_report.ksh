...
stmt="SELECT SRC_SYS||','||SRC_SLS_DIV_CD||','||SRC_CUST_ACCT_ID||','||SUBSTR(ECOLAB_BUS_MON,1,4)||','||CHAR(SUM(EXTD_SLS_AMT))  
FROM CTCMSDS.ECO_SLS_LAND WHERE GLBL_CUST_ID IS NULL AND EXTD_SLS_AMT <> 0 
GROUP BY SRC_SYS, SRC_SLS_DIV_CD, SRC_CUST_ACCT_ID,SUBSTR(ECOLAB_BUS_MON,1,4)"
db2 -x "$stmt" > /usr/local/bin/informatica/SrcFiles/missing_customers.txt
